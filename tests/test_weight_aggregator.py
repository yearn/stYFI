...
def test_activate_stake(chain, deployer, alice, bob, yfi, styfi, genesis, srd, lls, ll_depositors, llrd, ybc_aggregator, aggregator, styfi_middleware, ll_middlewares):
    ...
    with chain.isolate():
        ...
        assert aggregator.supply() == 15 * UNIT
        assert aggregator.packed_weights(alice) == 0
        assert aggregator.staked(alice) == 13 * UNIT
        assert aggregator.weight(alice) == 13 * UNIT
        ...
        packed = aggregator.packed_weights(alice)
        assert (packed >> 240) == 4 # epoch
        mask = (1 << 60) - 1
        assert packed >> 180 & mask == 13 * PACKED_UNIT
        assert packed >> 120 & mask == 13 * PACKED_UNIT + PACKED_UNIT // 2
        assert packed >> 60 & mask == 14 * PACKED_UNIT
        assert packed & mask == 14 * PACKED_UNIT + PACKED_UNIT // 2
        ...
        chain.mine()
        assert aggregator.weight(alice) == 13 * UNIT + UNIT // 2

        chain.pending_timestamp = genesis + EPOCH_LENGTH * 6
        chain.mine()
        assert aggregator.weight(alice) == 14 * UNIT

        lls[0].mint(alice, 4 * UNIT, sender=deployer)
        ...
        assert ybc_aggregator.last_stake() == (alice, alice, 17 * UNIT, 15 * UNIT, 2 * UNIT)

        chain.pending_timestamp = genesis + EPOCH_LENGTH * 7
        chain.mine()
        assert aggregator.weight(alice) == 15 * UNIT
